...
```{r}
setdiff(district_total$District, dhb_shp$HD_L2_20_1)
setdiff(dhb_shp$HD_L2_20_1, district_total$District)
```
```{r}
library(stringi)

dhb_shp <- dhb_shp %>%
  mutate(HD_L2_20_1_clean = stri_trans_general(HD_L2_20_1, "Latin-ASCII"))
```
...
```{r}
map_data <- dhb_shp %>%
  left_join(district_total, by = c("HD_L2_20_1_clean" = "District"))
```
...
map_data %>%
  st_drop_geometry() %>%
  count(HD_L2_20_1_clean) %>%
  arrange(desc(n))
```

## Figure 5: Total Initial Dispensings by District (Map)
...
map_data_ppl <- dhb_shp %>%
  left_join(district_total_ppl, by = c("HD_L2_20_1_clean" = "District"))
```
...
```
```{r}
ratio_data <- district_total %>%
  left_join(district_total_ppl, by = "District") %>%
  mutate(disps_per_person = total_dispensings / total_ppl) %>%
  arrange(desc(disps_per_person))
